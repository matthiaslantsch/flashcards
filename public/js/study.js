$(function(){
	...
	$("#skipBtn").on("click", function() {
		...
		if($("#checkBtn").prop("disabled")) {
			...
			sendUpdate(currentItem.idBox, currentItem.idCard, "CORRECTION");
			$(".alert").hide();
...
function verifyAnswer() {
	var currentItem = $("#studyField").data("currentItem");
	if(checkAnswer($("#answerField").val().trim())) {
		sendUpdate(currentItem.idBox, currentItem.idCard, "RIGHT");
		$(".alertArea").html('<div class="alert alert-success">Correct!</div>');
		$(".alert").fadeOut(800, nextCard);
	} else {
		sendUpdate(currentItem.idBox, currentItem.idCard, "WRONG");
